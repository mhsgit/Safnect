...
import com.populstay.common.bytearraytoint
...
object BLEByteArrayUtil {
    ...
    fun calcSendbyteArray(byteArray: ByteArray): Array<ByteArray?> {
        Log.d(GlobalConstant.APP_TAG, " calcSendbyteArray 原始数据长度byteArray = ${byteArray.size}")
        //根据当前的传输MTU值计算要分的包数
        //分包格式前前两个byte是总包数，当前包数,
        //为了节省字节，前4个字节为总包数2个，当前包数2个，
        //采用short的取值范围65536，分包如果超过这个总包数，就不做传输了
        val everybytelen = BlueToothBLEUtil.mtuSize - 4 -10
        val totalpkgs =
            byteArray.size / everybytelen + if (byteArray.size % everybytelen > 0) 1 else 0

        Log.d(GlobalConstant.APP_TAG, " calcSendbyteArray 总包数totalpkgs = ${totalpkgs}，包大小everybytelen = $everybytelen")
